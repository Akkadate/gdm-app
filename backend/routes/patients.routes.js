  ...
  try {
    ...
    if (role === 'patient') {
      ...
      if (patientResult.rows[0].id !== parseInt(req.params.id)) {
        return res.status(403).json({ message: 'ไม่มีสิทธิ์เข้าถึงข้อมูลผู้ป่วยรายนี้' });
      }
    }
...
router.put(
  ...
  [
    ...
    check('expected_delivery_date', 'รูปแบบวันกำหนดคลอดไม่ถูกต้อง').optional().isDate(),
    check('pre_pregnancy_weight', 'น้ำหนักก่อนตั้งครรภ์ต้องเป็นตัวเลข').optional().isNumeric(),
...
router.post(
  ...
  async (req, res) => {
    ...
    const { target_type, min_value, max_value, effective_date, notes } = req.body;
    ...
    try {
      ...
      const result = await req.db.query(
        ...
        [req.params.id, target_type, min_value, max_value, req.user.id, effective_date, notes]
      );
