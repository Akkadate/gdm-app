...
export const AuthProvider = ({ children }) => {
  ...
  const register = async (userData) => {
    try {
      ...
      return { success: false, error: err.response?.data?.message || 'เกิดข้อผิดพลาดในการลงทะเบียน' };
    }
  };
  ...
  const login = async (credentials) => {
    try {
      ...
      return { success: false, error: err.response?.data?.message || 'เกิดข้อผิดพลาดในการเข้าสู่ระบบ' };
    }
  };
  ...
  const isNurseOrAdmin = () => {
    return currentUser && (currentUser.role === 'nurse' || currentUser.role === 'admin');
  };
