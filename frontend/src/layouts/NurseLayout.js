...
import { FaHome, FaUserInjured, FaChartLine, FaCog, FaSignOutAlt, FaBars, FaTimes } from 'react-icons/fa';
...
const NurseLayout = () => {
  ...
  const nurseMenuItems = [
    { to: '/nurse/dashboard', icon: <FaHome />, text: 'แดชบอร์ด' },
    { to: '/nurse/patients', icon: <FaUserInjured />, text: 'รายชื่อผู้ป่วย' },
    { to: '/nurse/reports', icon: <FaChartLine />, text: 'รายงาน' }
  ];

  // เพิ่มเมนูสำหรับแอดมินถ้าผู้ใช้เป็นแอดมิน
  const isAdmin = currentUser && currentUser.role === 'admin';
  if (isAdmin) {
    nurseMenuItems.push({ to: '/nurse/settings', icon: <FaCog />, text: 'การตั้งค่า' });
  }
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="hidden md:flex flex-col w-64 bg-white shadow">
        <div className="flex items-center justify-center h-20 shadow-md">
          <h1 className="text-xl font-bold text-purple-600">GDM Care - Staff</h1>
        </div>
            ...
            {nurseMenuItems.map((item) => (
              <Link
                ...
                className={`flex items-center px-4 py-2 text-gray-700 rounded-lg ${
                  isActive(item.to) ? 'bg-purple-100 text-purple-700' : 'hover:bg-gray-100'
                }`}
              >
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <header className="flex items-center justify-between h-16 bg-white shadow px-6">
          <div className="flex items-center md:hidden">
            ...
            <h1 className="ml-4 text-xl font-bold text-purple-600 md:hidden">GDM Care - Staff</h1>
          </div>
          <div className="flex items-center">
            <span className="font-medium">
              {currentUser?.role === 'admin' ? 'แอดมิน:' : 'พยาบาล:'} {currentUser?.first_name || 'ผู้ใช้งาน'}
            </span>
          </div>
        </header>
